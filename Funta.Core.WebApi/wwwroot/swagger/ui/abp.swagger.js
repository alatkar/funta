...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        if (!usernameOrEmailAddress) {
            alert('Username or PhoneNumber is required, please try with a valid value !');
            return false;
        }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    //result.expireInSeconds = 100000;
                    var expireDate = new Date(Date.now() + (/*responseJSON.expireInSeconds*/ 100000 * 100000));
        ...
        //xhr.open('POST', 'http://localapi.apptherapist.ir:2021/api/Account/login', true);
        xhr.open('POST', 'http://localhost:2021/api/Account/login', true);
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
        ...
        createInput(modalUxContent, 'userName', 'Username or PhoneNumber');
